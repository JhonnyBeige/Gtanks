...
public class ProtocolTransfer {
    ...
    public void decryptProtocol(String protocol) {
        if ((this.inputRequest = new StringBuffer(protocol)).length() > 0) {
            if (this.inputRequest.toString().endsWith("end~")) {
                ...
                boolean localhostConnection = LocalhostBypass.isLocalhost(this.channel.getRemoteAddress());
                ...
                for(int var3 = 0; var3 < var4; ++var3) {
                    ...
                    try {
                        ...
                        if (localhostConnection) {
                            Logger.log("[EXCEPTION] Detected cheater(replace protocol): " + this.channel.toString() + " [localhost ignored]");
                            continue;
                        }
                        if (this.handleProtocolViolation("[EXCEPTION] Detected cheater(replace protocol): ")) {
                            return;
                        }
                        continue;
                    }

                    if (!localhostConnection && key == this._lastKey) {
                        if (this.handleProtocolViolation("Detected cheater(replace protocol): ")) {
                            return;
                        }
                        continue;
                    }
                    ...
                    if (!localhostConnection && key != (nextKey == 0 ? 1 : nextKey)) {
                        if (this.handleProtocolViolation("[NOT QUEQUE KEY " + nextKey + " " + this._lastKey + "] Detected cheater(replace protocol): ")) {
                            return;
                        }
                        continue;
                    }
    ...
    private boolean handleProtocolViolation(String prefix) {
        String ip = LocalhostBypass.extractIp(this.channel.getRemoteAddress());
        if (LocalhostBypass.isLocalhost(this.channel.getRemoteAddress())) {
            Logger.log(prefix + this.channel.toString() + " [localhost ignored]");
            return false;
        }
        Logger.log(prefix + this.channel.toString());
        NettyUsersHandler.block(ip);
    ...
    private String decrypt(String request, int key) {
        ...
        for(int i = 0; i < request.length(); ++i) {
            _chars[i] = (char)(_chars[i] - ((char)key + this.num));
        }
